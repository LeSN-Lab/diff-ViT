...
def get_activations(
    images, model, bit_config, device, normalize_act=False, layer_indices=None
):
    ...
    sorted_indices = sorted(
        range(len(layer_info)), key=lambda k: layer_info[k]["absolute_index"]
    )
    ...
    # layer_indices가 지정된 경우, 해당하는 activation들만 선택
    ...
    if layer_indices is None:
        return activations
    return activations, layer_info
    # 정렬된 레이어 정보 출력

    import pickle


def compute_cka_internal(
    model,
    use_batch=True,
    ...
    if use_batch:
        for index in range(cka_iter):
            ...
            for images, _ in cka_dataset:
                model_get_activation = get_activations(
                    images, model, None, normalize_act
                )  # 각 모델의 레이어별 활성화를 가져온다.

                cka.update_state(
                    model_get_activation
                )  # 레이어 마다의 activation을 다 가져옴. 예를 들어 24 * 50 * feature^2.
...
def compute_cka_with_adversarial(
    model1,
    model2,
    use_batch=True,
    normalize_act=False,
    ...
    if use_batch:
        for index in range(cka_iter):
            # cka_batch만큼, shuffle해서, 데이터셋을 가져온다.
            cka_dataset = get_dataset(n=cka_batch, args=args)
            current_iter = 0
            for images, labels in cka_dataset:
                ...
                cka.update_state(
                    model1_activations=model1_get_activation,
                    model1_adv_activations=model1_get_adv_activation,
                    model2_activations=model2_get_activation,
                    model2_adv_activations=model2_get_adv_activation,
                )  # 레이어 마다의 activation을 다 가져옴. 예를 들어 24 * 50 * feature^2.
        ...
        for images, labels in cka_dataset:
            ...
            all_adv_images = cka_attack_net1.gen_adv_inputs(
                inputs=all_images, labels=all_labels
            )
        cka.update_state(
