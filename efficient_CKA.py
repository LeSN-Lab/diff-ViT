...
class MinibatchCKA(nn.Module):
    ...
    def update_state(self, activations):
        ...
        layer_grams = torch.stack([self._generate_gram_matrix(x) for x in activations])
        # assign_add를 PyTorch의 in-place 덧셈으로 변경
        #평균이 0인 것을 계속 더한다. 레이어별 유사도 (24, 2500), (24, 2500)을 내적하면 각 레이어마다의 유사도가 나온다.
        self.hsic_accumulator.data.add_(torch.matmul(layer_grams, layer_grams.t())) 
    ...
    def result(self):
        ...
        if self.across_models:
            ...
            # diag_part를 diagonal로 변경
            normalization = torch.sqrt(mean_hsic.diagonal())
            # 인덱싱 방식을 PyTorch에 맞게 변경
            mean_hsic = mean_hsic / normalization.unsqueeze(1)
            mean_hsic = mean_hsic / normalization.unsqueeze(0)
        return mean_hsic